...
export async function seedItems(): Promise<void> {
  // Use pool.query() per item so each connection is auto-released immediately.
  // This avoids holding a single client for the full loop (171 items), which
  // previously triggered the pool sweep's force-kill → double-release crash.
