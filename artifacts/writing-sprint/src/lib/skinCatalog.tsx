...
import { RoadCar, RaceKart } from "./raceVehicles";
...
const ROAD_STYLES: Record<string, RoadStyle> = {
  mushroom: {
    trackBg: "linear-gradient(180deg, #354355 0%, #293646 100%)",
    kartSky: "linear-gradient(180deg, #28354c, #222e43)",
    kartRoad: "linear-gradient(180deg, #354156, #253146)",
    topKerb: "repeating-linear-gradient(90deg, #cf8790 0px, #cf8790 18px, #e4e7ef 18px, #e4e7ef 36px)",
    bottomKerb: "repeating-linear-gradient(90deg, #829ccc 0px, #829ccc 18px, #e4e7ef 18px, #e4e7ef 36px)",
...
export function SkinnedCar({ skinKey, color }: { skinKey: string | null | undefined; color?: string }) {
  ...
  return <RoadCar {...p} car={color ?? p.car} shade={color ?? p.shade} light={color ?? p.light} />;
}

export function SkinnedKart({ skinKey, laneNum, fallbackColor, fallbackShade, fallbackLight }: {
  skinKey: string | null | undefined; laneNum: number; fallbackColor?: string; fallbackShade?: string; fallbackLight?: string;
}) {
  const p = getCarPalette(skinKey);
  return <RaceKart {...p} car={fallbackColor ?? p.car} shade={fallbackShade ?? p.shade} light={fallbackLight ?? p.light} laneNum={laneNum} />;
}
