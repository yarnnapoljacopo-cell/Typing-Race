...
var CONFIG_DIR = import_path.default.join(import_electron.app.getPath("userData"), "writing-sprint");
...
function saveSettings(s) {
  ...
  import_fs.default.writeFileSync(CONFIG_FILE, JSON.stringify(s, null, 2));
}
function readSprints() {
  try {
    if (import_fs.default.existsSync(SPRINTS_FILE)) {
      return JSON.parse(import_fs.default.readFileSync(SPRINTS_FILE, "utf-8"));
    }
  } catch {
  }
  return [];
...
function writeSprints(sprints) {
  import_fs.default.mkdirSync(CONFIG_DIR, { recursive: true });
  import_fs.default.writeFileSync(SPRINTS_FILE, JSON.stringify(sprints, null, 2));
}
...
function checkOnline(url) {
  ...
  return new Promise((resolve) => {
    ...
    const req = mod.request(
      ...
      (res) => resolve(res.statusCode !== void 0 && res.statusCode < 400)
    );
...
function getOfflinePath() {
  const inAsar = import_path.default.join(process.resourcesPath, "offline.html");
  const inDev = import_path.default.join(__dirname, "../src/offline.html");
  return import_fs.default.existsSync(inAsar) ? inAsar : inDev;
}
async function createWindow() {
  currentSettings = loadSettings();
  import_electron.nativeTheme.themeSource = currentSettings.theme ?? "system";
...
import_electron.ipcMain.handle("save-file", async (_e, { content, defaultName }) => {
  if (!mainWindow) return { saved: false };
  const { filePath, canceled } = await import_electron.dialog.showSaveDialog(mainWindow, {
...
import_electron.ipcMain.handle("save-offline-sprint", (_e, { duration, words, text, goal }) => {
...
import_electron.ipcMain.handle("get-offline-sprints", () => {
  return readSprints().sort((a, b) => b.created_at.localeCompare(a.created_at));
});
import_electron.ipcMain.handle("delete-offline-sprint", (_e, { id }) => {
  const sprints = readSprints().filter((s) => s.id !== id);
  writeSprints(sprints);
  return { ok: true };
});
import_electron.ipcMain.handle("is-online", async () => {
  isOnline = await checkOnline(currentSettings.serverUrl);
  return isOnline;
});
import_electron.ipcMain.handle("go-online", async () => {
  isOnline = await checkOnline(currentSettings.serverUrl);
...
import_electron.ipcMain.handle("go-offline", async () => {
  if (mainWindow) {
    await mainWindow.loadFile(getOfflinePath());
    buildMenu();
  }
...
import_electron.ipcMain.handle("get-settings", () => loadSettings());
import_electron.ipcMain.handle("save-settings", (_e, settings) => {
  saveSettings(settings);
  currentSettings = settings;
  import_electron.nativeTheme.themeSource = settings.theme ?? "system";
  return { ok: true };
});
function buildMenu() {
  const isMac = process.platform === "darwin";
  const template = [
    ...isMac ? [{ label: import_electron.app.name, submenu: [
      { role: "about" },
      { type: "separator" },
      { role: "services" },
      { type: "separator" },
      ...
      submenu: [
        {
          ...
          click: async () => {
            ...
            try {
              ...
              if (!text || !text.trim()) {
                import_electron.dialog.showMessageBox(mainWindow, { type: "info", message: "No writing found in the current page." });
                return;
              }
              const { filePath, canceled } = await import_electron.dialog.showSaveDialog(mainWindow, {
                defaultPath: `sprint-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.txt`,
                filters: [{ name: "Text Files", extensions: ["txt"] }, { name: "All Files", extensions: ["*"] }]
              });
              if (!canceled && filePath) {
                import_fs.default.writeFileSync(filePath, text, "utf-8");
                import_electron.dialog.showMessageBox(mainWindow, { type: "info", message: `Saved to ${filePath}` });
              }
            } catch {
            }
          }
    ...
    },
    {
      ...
      submenu: [
        ...
        { type: "separator" },
        {
          ...
          click: () => import_electron.shell.openExternal(currentSettings.serverUrl)
        }
      ]
    }
  ];
...
function setupAutoUpdater() {
  if (!import_electron.app.isPackaged || process.platform === "darwin") return;
  ...
  import_electron_updater.autoUpdater.on("update-downloaded", (info) => {
    ...
    import_electron.dialog.showMessageBox(mainWindow, {
      ...
      detail: "The update installs automatically when you quit, or restart now to apply it immediately.",
...
import_electron.app.whenReady().then(() => {
  if (process.env.NODE_ENV === "development") {
    import_electron.session.defaultSession.setCertificateVerifyProc((_req, cb) => cb(0));
  }
  createWindow();
  setupAutoUpdater();
});
